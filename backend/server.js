...
app.use(cors({
  origin: [
    'http://localhost:3000',
    'https://whatsappghl-backend.vercel.app',
    'https://whatsappghl-backend.vercel.app',
...
app.get('/ghl/provider/config', (req, res) => {
  res.json({
    ...
    provider: {
      ...
      webhook_url: `${process.env.BACKEND_URL || 'https://whatsappghl-backend.vercel.app'}/ghl/provider/webhook`
    },
    settings: {
      webhook_url: {
        type: "url",
        ...
        required: true,
        default: `${process.env.BACKEND_URL || 'https://whatsappghl-backend.vercel.app'}/ghl/provider/webhook`
      }
    }
  });
});
      ...
      <html>
          <body style="font-family: Arial, sans-serif; padding: 20px; text-align: center;">
            ...
            <code style="background: #f0f0f0; padding: 10px; border-radius: 5px;">
              ${process.env.BACKEND_URL || 'https://whatsappghl-backend.vercel.app'}/ghl/provider?locationId=YOUR_LOCATION_ID
            </code>
...
app.post('/debug/test-incoming', async (req, res) => {
  try {
    ...
    const webhookResponse = await fetch(`${process.env.BACKEND_URL || 'https://whatsappghl-backend.vercel.app'}/whatsapp/webhook`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
...
app.post('/debug/test-outbound', async (req, res) => {
  try {
    ...
    // Call the webhook internally
    const webhookResponse = await fetch(`${process.env.BACKEND_URL || 'https://whatsappghl-backend.vercel.app'}/webhooks/ghl/provider-outbound`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
...
app.post('/api/test-team-notification', async (req, res) => {
  try {
    ...
    const notificationResponse = await fetch(`${process.env.BACKEND_URL || 'https://whatsappghl-backend.vercel.app'}/api/team-notification`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
